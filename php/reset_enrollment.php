<?php
/**
 * Reset Enlistment Status
 * For testing purposes - resets a student's enlistment status
 */
...
try {
    ...
    // Update enlistment status
    $stmt = $pdo->prepare("UPDATE students SET enrollment_status = 'Not Enlisted' WHERE student_id = ?");
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Enlistment status reset successfully. You can now enlist again.'
    ]);
    ...
    error_log("Reset enlistment error: " . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'An error occurred while resetting enlistment.']);
}
