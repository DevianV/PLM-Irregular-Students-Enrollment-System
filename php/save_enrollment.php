<?php
/**
 * Save Enlistment Module
 * Handles enlistment finalization with database transaction
 */
...
function saveEnrollment($student_id, $selected_subjects, $semester) {
    ...
    try {
        ...
        if ($stmt->fetchColumn() > 0) {
            return [
                'success' => false,
                'message' => 'You are already enlisted for this semester. Cannot create duplicate enlistment.'
            ];
        }
        ...
        // Update student enlistment status
        $stmt = $pdo->prepare("UPDATE students SET enrollment_status = 'Enlisted' WHERE student_id = ?");
        $stmt->execute([$student_id]);
        ...
        error_log("Enlistment error: " . $e->getMessage());
        return [
            'success' => false,
            'message' => 'An error occurred during enlistment. Please try again.'
        ];
    }
}
