...
function finalizeEnrollment() {
    ...
    summaryHTML += '<h4>Enlistment Summary</h4>';
...
function submitEnrollment() {
    ...
    .then(data => {
        if (data.success) {
            showAlert(data.message || 'Enlistment successful!', 'success');
            setTimeout(() => {
                window.location.href = 'dashboard.php';
            }, 2000);
        } else {
            showAlert(data.message || 'Enlistment failed.', 'error');
            submitBtn.disabled = false;
            submitBtn.textContent = 'Yes, Finalize Enlistment';
